...
from zoofs import ParticleSwarmOptimization, DragonFlyOptimization, GreyWolfOptimization
...
def plot_pca(X, y, title):
    ...
    tmp['target'] = y
    plt.scatter(tmp['principal component 1'], tmp['principal component 2'], c=tmp['target'])
    plt.title(title)
    plt.show()
...
def feature_selection(data, target):
    ...
    pso = ParticleSwarmOptimization(objective_function_topass,n_iteration=30,population_size=20,minimize=True)
    lgb_model = lgb.LGBMClassifier()                                       
    pso.fit(lgb_model,X_train, y_train, X_test, y_test,verbose=True)
    pso.plot_history()
    
    pso_feat = pso.best_feature_list
    
    
    da = DragonFlyOptimization(objective_function_topass,n_iteration=25,population_size=20,minimize=True)
    lgb_model3 = lgb.LGBMClassifier()                                       
    da.fit(lgb_model3,X_train, y_train, X_test, y_test,verbose=True)
    da.plot_history()
    
    da_feat = da.best_feature_list
    
    gwo = GreyWolfOptimization(objective_function_topass,n_iteration=25,population_size=20,minimize=True)
    lgb_model2 = lgb.LGBMClassifier()                                       
    gwo.fit(lgb_model2,X_train, y_train, X_test, y_test,verbose=True)
    gwo.plot_history()
    
    gwo_feat = gwo.best_feature_list
    return pso_feat, gwo_feat, da_feat
    


def main():
    filename = 'datasets/dermatology.csv'
    target = 'Class'
    data = pd.read_csv(filename, header=0)
    cat_features = []
    
    pso_feat, gwo_feat, da_feat = feature_selection(data, target)
    # num_features = pso_feat
    num_features = ['Erythema', 'Scaling', 'Definite_borders', 'Itching',
       ...
       'Inflammatory_monoluclear', 'Band-like_infiltrate', 'Age']
    ...
    # clf_model=SVC(C=2.5147504042065405, kernel= 'poly', coef0=0.058966526117779895)
    # clf_model = SVC()
